...
- Before creating a draft, the Worker checks Buzzsprout's complete episode list for the `pbc-youtube-VIDEO_ID` GUID and reuses a match. Failed, malformed, incomplete or ambiguous lookups stop without creating anything.
- If draft creation has an uncertain outcome, retry first performs a read-only lookup. A matching episode resumes with its confirmed audio/publication state. If none is found, the portal waits at least a minute after a new uncertain attempt and asks for confirmation after a manual Buzzsprout check. Close other publishing tabs first. This is not a cross-browser/server-side idempotency guarantee; never clear browser storage to bypass the check.
- Definite rejections allow another checked attempt. HTTP status and bounded, redacted API error details are shown and saved for troubleshooting instead of being replaced by a generic lock message. Network failures and unknown responses remain uncertain.
- If you uploaded the podcast manually, do **not** retry that sermon's podcast step in the portal. Manually created episodes do not carry the portal's matching GUID. Leave that workflow alone and start with a new video file for the next sermon.
